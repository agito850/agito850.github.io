...
雙精度浮點的「可表示整數」並不是連續的，它們之間有間隔（ULP，unit in the last place），而這個間隔會**每跨過一個 2 的次方就翻倍**：

| 數值範圍 | 相鄰可表示整數的間隔 | 結果 |
| --- | --- | --- |
| `0` ~ `2⁵³` | 1 | 每個整數都能精準表示 ✅ |
| `2⁵³` ~ `2⁵⁴` | **2** | 只剩偶數可表示，奇數被吸到鄰近偶數 |
| `2⁵⁴` ~ `2⁵⁵` | **4** | 連「不是 4 倍數」的偶數也開始壞 |

所以當 id 落在 `2⁵³ ~ 2⁵⁴` 這一段時，間隔正好是 **2**——偶數剛好踩在格子上、毫髮無傷；奇數卡在格子中間，就被四捨五入（IEEE 754 預設「就近取偶」）硬是改成隔壁的偶數。於是：
...
「只有奇數壞掉」不是隨機災情，而是**精度間隔 = 2** 的必然結果。再往上飆過 `2⁵⁴`，間隔變 4，連一半的偶數都要跟著陪葬。😆
